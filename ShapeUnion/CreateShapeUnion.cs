namespace TDCPlugin10
{
    ...
    public class CreateShapeUnion : Plugin
    {
        ...
        public void Apply(CSScene obsolete, CSG sceneGraph, int deviceId, ref float[] userDataFloats, ref int[] userDataInts, ref string userDataString)
        {
            ...
            var result = this.DoStuff(sceneGraph, targetShape, toolShape);
        ...
        private static extern void CopyMemory([In, Out] CSGVector[] pdst, CBODataTypes.CSGVector[] psrc, int cb);

        [DllImport("kernel32.dll", EntryPoint = "RtlMoveMemory")]
        private static extern void CopyMemory([In, Out] CBODataTypes.CSGVector[] pdst, CSGVector[] psrc, int cb);

        [DllImport("kernel32.dll", EntryPoint = "RtlMoveMemory")]
        private static extern void CopyMemory([In, Out] CSGUV[] pdst, CBODataTypes.CSGUV[] psrc, int cb);

        [DllImport("kernel32.dll", EntryPoint = "RtlMoveMemory")]
        private static extern void CopyMemory([In, Out] CBODataTypes.CSGUV[] pdst, CSGUV[] psrc, int cb);
        ...
        private CSGShape DoStuff(CSG sceneGraph, CSGShape targetShape, CSGShape toolShape)
        {
            ...
            targetShape.x_GetGeometryBoolean(
                ref PointList,
                ref PointListCount,
                ref NormalList,
                ref NormalListCount,
            ...
            toolShape.x_GetGeometryBoolean(
                ref PointList2,
                ref PointList2Count,
                ref NormalList2,
                ref NormalList2Count,
                ref TextureCoordinateList2,
                ref TextureCoordinateList2Count,
                ref faceData2,
                ref faceDataCount2,
                ref faceColorList2,
                ref faceColorList2Count);
            ...
            CopyMemory(aPointList, transformedPointList, aPointList.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));
            Debugger.Break();

            CBODataTypes.CSGVector[] aNormalList = new CBODataTypes.CSGVector[NormalListCount];
            CopyMemory(aNormalList, NormalList, aNormalList.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));

            CBODataTypes.CSGUV[] aTextureCoordinateList = new CBODataTypes.CSGUV[TextureCoordinateListCount];
            CopyMemory(aTextureCoordinateList, TextureCoordinateList, aTextureCoordinateList.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGUV)));

            CBODataTypes.CSGVector[] aPointList2 = new CBODataTypes.CSGVector[PointList2Count];
            CopyMemory(aPointList2, transformedPointList2, aPointList2.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));

            CBODataTypes.CSGVector[] aNormalList2 = new CBODataTypes.CSGVector[NormalList2Count];
            CopyMemory(aNormalList2, NormalList2, aNormalList2.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));

            CBODataTypes.CSGUV[] aTextureCoordinateList2 = new CBODataTypes.CSGUV[TextureCoordinateList2Count];
            CopyMemory(aTextureCoordinateList2, TextureCoordinateList2, aTextureCoordinateList2.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGUV)));
            ...
            if (lFaceMax3 > 0)
            {
                ...
                CopyMemory(cvaCompactedVertices3, acvaCompactedVertices3, cvaCompactedVertices3.Length * Marshal.SizeOf(typeof(CSGVector)));
