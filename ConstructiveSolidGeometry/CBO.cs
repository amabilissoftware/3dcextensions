...
namespace ACBO
{
    ...
    public class CBO
    {
            ...
            ref int[] faceColor)
        {
            ...
            CopyMemory(PointListA, PointList, PointList.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));

            Vector3[] NormalListA = new Vector3[NormalList.Length];
            CopyMemory(NormalListA, NormalList, NormalList.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));

            Vector3[] PointListA2 = new Vector3[PointList2.Length];
            CopyMemory(PointListA2, PointList2, PointList2.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));

            Vector3[] NormalListA2 = new Vector3[NormalList2.Length];
            CopyMemory(NormalListA2, NormalList2, NormalList2.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));

            Vector3[] cvaCompactedVerticesA = new Vector3[0];

            this.ProcessToolOperation2(
                udtOperation,
                ref PointListA,
                PointListCount,
                ref NormalListA,
            ...
            CopyMemory(cvaCompactedVertices, cvaCompactedVerticesA, cvaCompactedVerticesA.Length * Marshal.SizeOf(typeof(CBODataTypes.CSGVector)));
        ...
        public bool RayIntersectsBox(
            ref CBODataTypes.CSGVector rayPosition,
            ref CBODataTypes.CSGVector rayDirection,
            ref CBODataTypes.CSGVector boxMinimum,
            ref CBODataTypes.CSGVector boxMaximum,
            ref CBODataTypes.CSGVector intersectionPosition)
        {
        ...
        public bool RayIntersectsTriangle(
            ref CBODataTypes.CSGVector rayPosition,
            ref CBODataTypes.CSGVector rayDirection,
            ref CBODataTypes.CSGVector vertex0,
            ref CBODataTypes.CSGVector vertex1,
            ref CBODataTypes.CSGVector vertex2,
            ref CBODataTypes.CSGVector intersectionPosition)
        {
            Ray raySDX = new Ray(new Vector3(rayPosition.X, rayPosition.Y, rayPosition.Z), new Vector3(rayDirection.X, rayDirection.Y, rayDirection.Z));
            ...
            var wasTriangleIntersected = Collision.RayIntersectsTriangle(ref raySDX, ref vertexSDX0, ref vertexSDX1, ref vertexSDX2, out intersectionPositionSDX);
            ...
            ref float fltPlaneIntersectionPositionZ)
        {
            ...
            if (fltTScalerDenominator >= 0.00001 || fltTScalerDenominator <= -0.00001)
            {
                ...
                float fltTScalerNumerator = -(fltPlaneNormalX * fltRaydvPosX + fltPlaneNormalY * fltRaydvPosY + fltPlaneNormalZ * fltRaydvPosZ + fltDScaler);
        ...
        private static extern void CopyMemory([In, Out] Vector3[] pdst, CBODataTypes.CSGVector[] psrc, int cb);

        [DllImport("kernel32.dll", EntryPoint = "RtlMoveMemory")]
        private static extern void CopyMemory([In, Out] CBODataTypes.CSGVector[] pdst, Vector3[] psrc, int cb);
        ...
        // Parameters:
        // *********************************************************************************
        private static bool pbExteriorFace(CBODataTypes.CSGRay xdtRay, ref ConstructiveSolidGeometryMesh udtExterior, bool bCoplanarExterior, bool bCoplanarOppositeExterior)
        {
            ...
            if (bExteriorPick && Helper.bVectorEqual(
                    xdtRay.Position.X,
                    xdtRay.Position.Y,
                    xdtRay.Position.Z,
            ...
            else
            {
                ...
                if (bInteriorPick && Helper.bVectorEqual(
                        xdtRay.Position.X,
                        xdtRay.Position.Y,
                        xdtRay.Position.Z,
        ...
        // Parameters:
        // *********************************************************************************
        private static bool pbIntersectProcessVertex(ref ConstructiveSolidGeometryMesh udtIntersectee, ref ConstructiveSolidGeometryMesh udtIntersector, int lFace, int lVertex)
        {
            bool result = false;
            ...
            if (sEdgeLength >= Helper.CSG_sSmallEpsilon * 10)
            {
                ...
                if (bPick)
                {
                    ...
                    if (udtaPick[lPick].bConvex)
                    {
                        ...
                        result = udtIntersector.bSplitIntersectorEdge(lFace, lVertex, cvPickedPosition.X, cvPickedPosition.Y, cvPickedPosition.Z, bEdgeComplete);
                    }
                    else
                    {
        ...
        // Parameters:
        // *********************************************************************************
        private static bool pbPreDetectExteriorFace(CBODataTypes.i_CSGPoint[] xdtVertex, CBODataTypes.CSGCGFace udtFace, ref ConstructiveSolidGeometryMesh udtCGMesh)
        {
            bool result = false;
            ...
            bool bInvertFace)
        {
            ...
            for (lVertex = 0; lVertex <= udtSourceCGMesh.udtFace[lSourceFace].lVertex.GetUpperBound(0); lVertex++)
            {
                ...
                {
                    // invert the face
                    xdtVertex = udtSourceCGMesh.xdtVertex[udtSourceCGMesh.udtFace[lSourceFace].lVertex[udtSourceCGMesh.udtFace[lSourceFace].lVertex.GetUpperBound(0) - lVertex]];
                    xdtVertex.NextPoint = 0;
                }
            ...
            for (int lFace = 0; lFace <= udtCGMeshB.udtFace.GetUpperBound(0); lFace++)
            {
                if (!udtCGMeshB.udtFace[lFace].bDeleted)
                {
                    ...
                    if (udtCGMeshB.udtFace[lFace].udtIntersection == CBODataTypes.CSGFaceType.CSGFaceUnknown)
                    {
                        ...
                        // testing
                        if (Math.Abs(xdtRay.Direction.X) < Helper.CSG_sSmallEpsilon && Math.Abs(xdtRay.Direction.Y) < Helper.CSG_sSmallEpsilon
        ...
        // Parameters:
        // *********************************************************************************
        private static void pIntersectProcessCGMesh(ref ConstructiveSolidGeometryMesh udtIntersectee, ref ConstructiveSolidGeometryMesh udtIntersector)
        {
            int lFace = 0;
            var lEdge = new int[2];
            ...
            ref ConstructiveSolidGeometryMesh udtResultCGMesh)
        {
            ...
            if (udtOperation != CBODataTypes.CSGToolOperation.CSGToolOpMerge)
            {
                ...
                while ((lCurrentTargetFaces != lLastTargetFaces || lCurrentToolFaces != lLastToolFaces) && !bIntersectionFailure && lAttempt < 10)
                {
            ...
            for (int lFace = 0; lFace <= udtTargetCGMesh.udtFace.GetUpperBound(0); lFace++)
            {
                ...
                if (!udtTargetCGMesh.udtFace[lFace].bDeleted)
                {
                    ...
                    if (udtTargetCGMesh.udtFace[lFace].udtIntersection == CBODataTypes.CSGFaceType.CSGFaceExterior)
                    {
                        ...
                        if (udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpCut || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpUnion
                            || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpDifference || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpBlend
                            || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpBridge || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpMerge)
                        {
                            ...
                            catch
                            {
                                ...
                                udtResultCGMesh.udtFace[udtResultCGMesh.udtFace.GetUpperBound(0)].udtIntersection = CBODataTypes.CSGFaceType.CSGFaceExterior;
                            }
                        }
                    }
                    else
                ...
                for (int lFace = 0; lFace <= udtToolCGMesh.udtFace.GetUpperBound(0); lFace++)
                {
                    ...
                    if (!udtToolCGMesh.udtFace[lFace].bDeleted)
                    {
                        ...
                        if (udtToolCGMesh.udtFace[lFace].udtIntersection == CBODataTypes.CSGFaceType.CSGFaceExterior)
                        {
                            ...
                            if (udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpUnion || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpBlend
                                || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpBridge || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpMerge)
                            {
                                // ignore any errors caused by the add of a face
                                try
                                ...
                                catch
                                {
                                    ...
                                    udtResultCGMesh.udtFace[udtResultCGMesh.udtFace.GetUpperBound(0)].udtIntersection = CBODataTypes.CSGFaceType.CSGFaceInterior;
                                }
                            }
                        }
                        else
                        {
                            ...
                            // the interior polygons
                            if (udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpIntersection || udtOperation == CBODataTypes.CSGToolOperation.CSGToolOpDifference)
                            {
